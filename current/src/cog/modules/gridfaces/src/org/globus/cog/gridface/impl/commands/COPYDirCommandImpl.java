...
public class COPYDirCommandImpl extends GridCommandImpl implements GridCommand {

	private TaskGraph taskgraph = null;
	private File tempFile = null;
	
	
	private GridCommand putCommand = new PUTDIRCommandImpl();
	private GridCommand getCommand = new GETDIRCommandImpl();
	
	
	public COPYDirCommandImpl() {
		super();
		setCommand("copydir");
	}
	...
	public ExecutableObject prepareTask() throws Exception {
		if (validate() == true) {
			...
			this.tempFile = File.createTempFile("temp", "dir");
			this.tempFile = new File(tempFile.getAbsolutePath() + "_DIR");
			this.tempFile.mkdir();
...
//				putTask.getIdentity());
	...
	private Task prepareGetTask() throws Exception {
		...
		getCommand.addArgument(tempFile.getAbsolutePath()+"/tmp");
	...
	private Task preparePutTask() throws Exception {
		...
		putCommand.addArgument(tempFile.getAbsolutePath()+"/tmp");
