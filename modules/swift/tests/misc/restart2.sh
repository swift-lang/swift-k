...
swift -tc.file tmp.restart.tc.data restart2.swift
...
if [ "$FIRSTEXIT" == 0 ]; then
  ...
  exit 1
elif  [ ! -f restart-1.out ] || [ ! -f restart-3.out ]; then
  echo Failed  - expected first round output does not exist
  exit 2
elif  [ -f restart-2.out ] ; then
...
swift -resume *.rlog -tc.file tmp.restart.tc.data restart2.swift
