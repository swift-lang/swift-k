...
public class MoabExecutor extends AbstractExecutor {
	public static final Logger logger = Logger.getLogger(MoabExecutor.class);
	...
	/**
	   Moab processes-per-node
	 */
	int ppn = 1;

        /** 
           Jobs per node (number of workers to start per node)
        */
        int jpn = 1;
	...
	*/
	...
	public MoabExecutor(Task task, ProcessListener listener) {
		super(task, listener);
	}
	...
	protected void validate(Task task) {
		...
		if (name == null) {
		    int i = 0;
            synchronized(MoabExecutor.class) {
                i = unique++;
            }
            name = "cog-" + IDF.format(i);
            if (logger.isDebugEnabled()) {
                logger.debug("Moab name: for: " + task.getIdentity() + 
                         " is: " + name);
            }
		}
	...
	throws IOException {
		...
		if (value != null) {
			wr.write("#MSUB " + arg + String.valueOf(value) + '\n');
		}
	}
	...
	throws IOException {
	    ...
	    if (o != null)
            ...
            o = spec.getAttribute("jobsPerNode");
            if(o != null) 
               jpn = parseAndValidateInt(o, "jobsPerNode");
	...
	throws IOException {
		...
		wr.write("#MSUB -S /bin/bash\n");
		wr.write("#MSUB -N " + task.getName() + '\n');
		writeNonEmptyAttr("project", "-A ", wr);
		boolean multiple = writeCountAndPPN(spec, wr);
		writeWallTime(wr);
		writeNonEmptyAttr("queue", "-q ", wr);
		wr.write("#MSUB -o " + quote(stdout) + '\n');
		wr.write("#MSUB -e " + quote(stderr) + '\n');
		...
		if (spec.getEnvironmentVariableNames().size() > 0) {
		    wr.write("#MSUB -v " + makeList(spec.getEnvironmentVariableNames()) + '\n');
		}
		...
		// Don't use srun for MPI jobs
                if("single".equalsIgnoreCase(type)) {
                        ...
                        wr.write("srun -n" + count + " --mpi=none /bin/bash -c \'");
                }
	...
	throws IOException {
		...
		writer.write("#CoG   by class: " + MoabExecutor.class + '\n');
	...
	protected void writeMultiJobPreamble(Writer wr, String exitcodefile)
            throws IOException {
        wr.write("NODES=$( scontrol show hostnames $SLURM_JOB_NODELIST )\n");
