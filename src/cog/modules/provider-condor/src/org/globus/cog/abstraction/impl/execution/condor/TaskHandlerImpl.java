...
public class TaskHandlerImpl implements TaskHandler, StatusListener {
    ...
    public void submit(Task task)
        ...
        if (task.getType() != Task.JOB_SUBMISSION){
            throw new TaskSubmissionException("Condor execution task handler can only handle job submission tasks");
        }
        DelegatedTaskHandler dth =
            new JobSubmissionTaskHandler();
    ...
    public void suspend(Task task)
        ...
        DelegatedTaskHandler dth =
            (DelegatedTaskHandler) this.handleMap.get(task);
        if (dth != null) {
            dth.suspend();
        }
    }
    ...
    public void remove(Task task) throws ActiveTaskException {
        ...
        if ((status == Status.ACTIVE)) {
            ...
            /* this is required because statusChange listeners are invoked in a single thread
             * So if one listener that wants to remove a task from the handler is invoked 
             * before the listener implemented by this object, the task will be be in the activeList
             * and will not be actually removed.
             * 
