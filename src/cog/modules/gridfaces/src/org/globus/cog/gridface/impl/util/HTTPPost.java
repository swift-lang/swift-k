...
public class HTTPPost  {
	...
	public String post(URL url, Map args) throws IOException {
		...
		while(iVarNames.hasNext()) {
			...
			if(valueObj instanceof String[]) {
				logger.info("instanceof String[]");
				values = (String[]) valueObj;									
			}else if(valueObj instanceof String) {
				logger.info("instanceof String");
				values = new String[] { (String)valueObj };
			}else {
				logger.debug("valueObj should be a String or String[] using String.valueOf(valueObj)");
			...
			for(int i=0;values!= null && i<values.length;i++) {
				...
				// put an & after each value, does not occur with the first parameter name
				if(!isFirst) {
					content.append("&");
				}else {
					isFirst = false;
	...
	public static void main(String[] args) {
		...
		try {
			...
			data.put("var1", new String[] { "value_1.1", "value_1.2",
					"value_1.3" });
