...
public class ReadVariable extends Node {
	final static Logger logger = Logger.getLogger(ReadVariable.class);

	private VarRef<AbstractDataNode> refStatic;
	...
	public void run(LWThread thr) {
		...
		if (refStatic != null) {
		    ...
		    h = w.getHandle();
		    //System.out.println(this + ": " + h.getDisplayableName());
		    int rc = w.decRefCount();
		    //System.out.println(this + ": " + h.getDisplayableName() + " -> " + rc);
		    if (rc == 0) {
		    	ref.setValue(stack, null);
		    }
		}
    ...
    public Node compile(WrapperNode wn, Scope scope) throws CompilationException {
        ...
        if (ref.isStatic()) {
            if (vargs.append(ref.getValue())) {
                ...
                this.refStatic = (VarRef<AbstractDataNode>) ref;
