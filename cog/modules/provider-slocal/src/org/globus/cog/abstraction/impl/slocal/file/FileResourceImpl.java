...
public class FileResourceImpl implements FileResource {
	...
	public FileResourceImpl() {
		...
		throw new SecurityException("The secure local file resource is not available.");
	}
	...
	/** equivalent to cd */
	public void setCurrentDirectory(String directory) throws DirectoryNotFoundException,
			GeneralException {
		try {
	...
	public Collection list(String directory) throws DirectoryNotFoundException, GeneralException {
		try {
			...
			throw new DirectoryNotFoundException("Could not find directory: " + directory + ne);
		}
		catch (Exception e) {
	...
	public void createDirectory(String directory) throws GeneralException {
		try {
			...
			throw new GeneralException("Could not create directory: " + directory);
		}
	}

	/** delete the given directory. If force == true, recursive delete */
	public void deleteDirectory(String directory, boolean force) throws DirectoryNotFoundException,
			GeneralException {
		...
		while ((fileNames != null) && (fileNames.hasNext()) && (force = true)) {
			File newFile = new File(((GridFile) fileNames.next()).getAbsolutePathName());
	...
	/** remove a file */
	public void deleteFile(String fileName) throws FileNotFoundException, GeneralException {
		try {
	...
	/** copy a file */
	public void getFile(String remoteFileName, String localFileName) throws FileNotFoundException,
			GeneralException {
		try {
		...
		try {
			...
			while ((fileNames != null) && (fileNames.hasNext())) {
				...
				if (gridFile.isFile() == true) {
					getFile(gridFile.getAbsolutePathName(), localDirName + File.separator
							+ gridFile.getName());
				}
				else {
					getDirectory(gridFile.getAbsolutePathName(), localDirName + File.separator
							+ gridFile.getName());
				}
			}
			...
			throw new DirectoryNotFoundException("Directory not found in getdir " + de);
		}
		catch (Exception e) {
	...
	public void getMultipleFiles(String[] remoteFileNames, String localDirName)
			throws FileNotFoundException, DirectoryNotFoundException, GeneralException {
		for (int i = 0; i < remoteFileNames.length; i++) {
	...
	private GridFile createGridFile(Object obj) throws GeneralException {
		...
		try {
			...
			gridFile.setLastModified(String.valueOf(new Date(fileInfo.lastModified())));
