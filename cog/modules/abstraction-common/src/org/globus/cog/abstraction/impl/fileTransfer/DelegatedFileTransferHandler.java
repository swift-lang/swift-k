...
public class DelegatedFileTransferHandler implements DelegatedTaskHandler, Runnable,
		UrlCopyListener {
	static Logger logger = Logger.getLogger(DelegatedFileTransferHandler.class.getName());
	...
	private void prepareTransfer() throws IllegalSpecException, InvalidSecurityContextException,
			InvalidServiceContactException, TaskSubmissionException {
		...
		try {
			...
			throw new IllegalSpecException("Exception while retreiving FileTransferSpecification",
					e);
		}

		Service sourceService = this.task.getService(Service.FILE_TRANSFER_SOURCE_SERVICE);
		Service destinationService = this.task.getService(Service.FILE_TRANSFER_DESTINATION_SERVICE);
		...
		if (spec.isThirdParty()) {
			if ((sourceService.getProvider().equalsIgnoreCase("gridftp")
					|| sourceService.getProvider().equalsIgnoreCase("gsiftp") || sourceService.getProvider().equalsIgnoreCase(
					"gridftp-old"))
					&& (destinationService.getProvider().equalsIgnoreCase("gridftp")
							|| destinationService.getProvider().equalsIgnoreCase("gsiftp") || destinationService.getProvider().equalsIgnoreCase(
							"gridftp-old"))) {
				this.thirdparty = true;
			}
			else {
				throw new IllegalSpecException("Third party transfers between providers "
						+ sourceService.getProvider() + " and " + destinationService.getProvider()
						+ " is not supported");
			}
		}
		else {
	...
	protected boolean canHandle(Service service) {
		return isLocal(service)
				|| AbstractionFactory.hasObject(service.getProvider(), "fileResource")
				|| AbstractionFactory.hasObject(service.getProvider(), "fileTransferTaskHandler");
	}
	...
	protected FileResource prepareService(Service service) throws InvalidProviderException,
		...
		else if (AbstractionFactory.hasObject(service.getProvider(), "fileResource")) {
			if (logger.isDebugEnabled()) {
				logger.debug("Starting service on " + service.getServiceContact());
			}
			...
			if (this.sourceResource != null) {
				if (this.sourceResource.isDirectory(spec.getSource())) {
					if (localDestination == null) {
						localDestination = File.createTempFile(
								Long.toString(this.task.getIdentity().getValue()), null);
				...
				transferWithHandler(service.getProvider(), service, LOCAL_SERVICE,
						spec.getSource(), localDestination);
		...
		if (isLocal(service)) {
			...
			if (this.destinationResource != null) {
				if (localSource.isDirectory()) {
					...
					this.destinationResource.putDirectory(localSource.getAbsolutePath(),
							spec.getDestination());
				}
				else {
					if (logger.isDebugEnabled()) {
					...
					this.destinationResource.putFile(localSource.getAbsolutePath(),
							spec.getDestination());
				}
			}
			else {
	...
	protected void transferWithHandler(String provider, Service sourceService,
		...
		try {
			...
			t.setService(Service.FILE_TRANSFER_DESTINATION_SERVICE, destinationService);
			FileTransferSpecification tspec = new FileTransferSpecificationImpl();

			if (source instanceof File) {
				tspec.setSourceDirectory(((File) source).getParentFile().getAbsolutePath());
	...
	public void run() {
		...
		if (this.thirdparty) {
			...
			try {
				...
				File intermediate = doSource(sourceService, getLocalDestination(destinationService));
				try {
	...
	private void doThirdPartyTransfer() {
		...
		try {
			String url = this.task.getService(Service.FILE_TRANSFER_SOURCE_SERVICE).getServiceContact().getContact()
					+ "/" + spec.getSource();
			...
			url = this.task.getService(Service.FILE_TRANSFER_DESTINATION_SERVICE).getServiceContact().getContact()
					+ "/" + spec.getDestination();
			if (!url.startsWith("gsiftp://")) {
				url = "gsiftp://" + url;
			}
		...
		SecurityContext securityContext = this.task.getService(Service.FILE_TRANSFER_SOURCE_SERVICE).getSecurityContext();
		try {
			urlCopy.setCredentials((GSSCredential) securityContext.getCredentials());
			Authorization authorization = (Authorization) securityContext.getAttribute("authorization");
	...
	public void transfer(long current, long total) {
		if (total == -1) {
			if (current == -1) {
				...
				this.task.setStdOutput(Long.toString(current) + " bytes transferred");
			}
		}
		else {
			logger.debug(current + " out of " + total + " bytes transferred");
			this.task.setStdOutput(current + " out of " + total + " bytes transfered");
		}
