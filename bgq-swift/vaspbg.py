...
def prebooted(subblock_size, args):
    ...
    sparg="runjob --block " + os.environ["COBALT_PARTNAME"] + " --corner " + corner + " --shape " + shape + " -p 4 --np " + str(4*int(subblock_size)) + " : " + args
...
def mixed(subblock_size, args):
    ...
    if not os.path.exists("/home/ketan/corners.txt"):
        with open("/home/ketan/corners.txt", "w") as cornersfile:
            ...
            blocks=subprocess.check_output(["get-bootable-blocks", "--size", "512", os.environ["COBALT_PARTNAME"]]).split()
    ...
    sparg="runjob --block " + block + " --corner " + corner + " --shape " + shape + " -p 4 --np " + str(4*int(subblock_size)) + " : " + args
    logging.info("sparg is %s", sparg)
...
def main():
    ...
    logging.info("sys.argv[1] is %s", sys.argv[1])

    incarfile   = sys.argv[2]
    logging.info("incarfile is %s", incarfile)
    poscarfile  = sys.argv[3]
    logging.info("poscarfile is %s", poscarfile)
    potcarfile  = sys.argv[4]
    logging.info("potcarfile is %s", potcarfile)
    kpointsfile = sys.argv[5]
    logging.info("kpointsfile is %s", kpointsfile)
    
    outcarfile  = sys.argv[6]
    contcarfile = sys.argv[7]
    
    subprocess.call(["cp", incarfile, "."])
    subprocess.call(["cp", poscarfile, "."])
    subprocess.call(["cp", potcarfile, "."])
    subprocess.call(["cp", kpointsfile, "."])

    setsysenv()
    #prebooted(os.environ["SUBBLOCK_SIZE"], args)
    mixed(os.environ["SUBBLOCK_SIZE"], sys.argv[1])
    logging.info("value of SUBBLOCK_SIZE is %s", os.environ["SUBBLOCK_SIZE"])
    
    subprocess.call(["mv", "./OUTCAR", outcarfile])
    subprocess.call(["mv", "./CONTCAR", contcarfile])
