...
#mfixbgsh_restart "/home/ketan/aytekinwork/swift/Pmfix.O3.GNU.mira" _mfix_args _indir _outdir _restartcount stdout=@_o stderr=@_e;
#    $0                                   $1                           $2         $3     $4         $5     
#set -x
...
then
    ...
    then
        ...
        runjob --block $BLOCK -p 8 --np "$((8*$SUBBLOCK_SIZE))" : $1 $2
        ...
        echo "$0": running cmd: "$0" args: "$@"
        echo "$0": runjob --block "$COBALT_PARTNAME" --corner "$CORNER" --shape "$SHAPE" -p 4 --np "$((4*$SUBBLOCK_SIZE))" : $1 $2
        
        runjob --block "$COBALT_PARTNAME" --corner "$CORNER" --shape "$SHAPE" -p 4 --np "$((4*$SUBBLOCK_SIZE))" : $1 $2
        #rm -f $indir/*.RES
...
mkdir -p $outdir
#cp $outfile $outdir/
#cp GASIFIER3D.* $outdir/ 
#cp GASIFIER3D_*.vtu $outdir/
#cp GASIFIER3D_boundary.vtk $outdir/
#cp *.log $outdir/
#cp *.LOG $outdir/
#cp *.TXT $outdir/
cp $indir/* $outdir/

#case "$countrestart" in
#0) sed 's/new/restart_1/g' $indir/mfix.dat > $outdir/mfix.dat
#;;
#1) sed 's/restart_1/restart_2/g' $indir/mfix.dat > $outdir/mfix.dat
#;;
#2) sed 's/restart_2/restart_3/g' $indir/mfix.dat > $outdir/mfix.dat
#;;
#*) echo " invalid value of countrestart "
#;;
#esac
